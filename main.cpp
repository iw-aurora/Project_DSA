...
// Hàm riêng nạp dữ liệu từ file JSON và trả về mảng dynamic array
// (vector<Student>)
vector<Student> loadStudentsData(const string &filePath) {
  ...
  if (!data.contains("students") || !data["students"].is_array()) {
    throw runtime_error(
        "File JSON khong dung dinh dang (thieu mang 'students').");
  }

  vector<Student> students;
  for (const auto &item : data["students"]) {
    students.push_back({item.value("id", ""), item.value("name", ""),
                        item.value("classId", ""), item.value("gpa", 0.0)});
  }

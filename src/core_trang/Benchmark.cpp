...
vector<string> MC1::Benchmark::createQueries(
    const vector<Student>& students) const
{
    ...
    for (size_t i = 0; i < 950; ++i)
        queries.push_back(
            students[(i * 37) % students.size()].id
        );
...
MC1::Benchmark::Result MC1::Benchmark::run(
...
void MC1::Benchmark::printResult(const Result& result) const
{
    ...
    cout << left
         << setw(28) << "So phep kiem tra / probe"
         << right
         << setw(18)
         << result.linearComparisons
         << setw(18)
